"""
Background Watcher: Monitors for completion of diffuse_scan_50um_scan_5.*
Once scan_5 finishes, it updates notes.txt with final metrics and pushes to GitHub.
"""
import os
import sys
import time
...
TARGET_PNG = os.path.join(DIFFUSE_DIR, "diffuse_scan_50um_scan_5.png")
TARGET_NPY = os.path.join(DIFFUSE_DIR, "diffuse_scan_50um_scan_5.npy")
TARGET_CSV = os.path.join(DIFFUSE_DIR, "diffuse_scan_50um_scan_5.csv")

print("=" * 70)
print("BACKGROUND WATCHER ACTIVE: Waiting for diffuse_scan_50um_scan_5 completion...")
    ...
    if os.path.exists(TARGET_PNG) and os.path.exists(TARGET_NPY):
        ...
        if initial_size > 100_000:  # Valid PNG figure is ~500KB+
            ...
            if os.path.getsize(TARGET_PNG) == initial_size:
                print("\n[DETECTED] Scan completed! diffuse_scan_50um_scan_5 files found and verified.", flush=True)
                break
    
    # Heartbeat every 30 minutes
    elapsed_hr = (time.time() - start_watch_time) / 3600.0
    if int(time.time() - start_watch_time) % 1800 < check_interval:
        print(f"[WATCHER RUNNING] Still monitoring for diffuse scan_5 completion... (Waiting {elapsed_hr:.1f} hrs)", flush=True)
        
    time.sleep(check_interval)

# --- PROCESS METRICS & UPDATE NOTES ---
print("\nCalculating metrics from diffuse_scan_50um_scan_5.npy...", flush=True)
try:
    ...
    - Files: diffuse_scan_50um_scan_5.* (saved in ZaberScan/results/diffuse/)"""
    ...
    if os.path.exists(NOTES_PATH):
        ...
        print("[OK] notes.txt successfully updated with final diffuse scan_5 metrics.", flush=True)
...
try:
    ...
    commit_msg = "Add results and notes for diffuse target 50um scan 5"
    ...
    print("\n[SUCCESS] Diffuse target 50um scan 5 results and notes successfully committed and pushed to GitHub!", flush=True)
